...
# 0:短波向上  1: 短波向下 2: 长波向上  3: 长波向下 
v_ref_true = np.asarray(targets_unnorm_list[3][:,3,0])[:,np.newaxis]
...
plt.ylabel("latitude" , fontsize = 22, font = font1)


for i in range(4):
    v_ref_predict = np.asarray(predicts_unnorm_list[i][:,3,0])[:,np.newaxis]
...
plt.xlabel("longitude" , fontsize = 22, font = font1)
    # axs[2,0].text(0.02,0.88,"(e)",font = font1,fontsize = 28)


for i in range(4):
    v_ref_predict = np.asarray(predicts_unnorm_list[i][:,3,0])[:,np.newaxis]
    v_ref_true  =    np.asarray(targets_unnorm_list[i][:,3,0])[:,np.newaxis]
