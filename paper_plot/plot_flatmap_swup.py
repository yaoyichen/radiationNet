...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 14 11:36:03 2022
...
v_ref_true = np.asarray(targets_unnorm_list[2][:,0,-1])[:,np.newaxis]
...
clb = fig.colorbar(cs1, orientation='vertical', 
                    ...
                    label = "Flux [W/m2]", 
                    ticklocation = "top"    ,
                    # ticks = [0,150,300,450,600,750,900,1050,1200,1350],
...
clb.set_label(r"Flux [$\mathrm{W \cdot m^{-2}}$]", fontsize = 18, labelpad=-10, 
...
for i in range(4):
    v_ref_predict = np.asarray(predicts_unnorm_list[i][:,0,-1])[:,np.newaxis]

    grid_x, grid_y, grid_v_predict = interpolate_scatter_to_grid(x_grid_vector, y_grid_vector, x_ref,y_ref,v_ref_predict)
    grid_v_predict = grid_v_predict.squeeze()
    
    grid_v_predict[grid_v_predict > v_max ] = v_max
    grid_v_predict[grid_v_predict < v_min ] = v_min
...
for i in range(4):
    v_ref_predict = np.asarray(predicts_unnorm_list[i][:,0,-1])[:,np.newaxis]
    v_ref_true  =    np.asarray(targets_unnorm_list[i][:,0,-1])[:,np.newaxis]
...
plt.xlabel("longitude" , fontsize = 20, font = font1)
    # plt.ylabel("latitude" , fontsize = 16, font = font1)
